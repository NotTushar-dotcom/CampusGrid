...
export default function UnassignedView({ currentUser, supabaseUserId, onTeamJoined }: UnassignedViewProps) {
  ...
  const loadTeams = useCallback(async (isInitial = false) => {
    if (!supabase) return;
    if (isInitial) setTeamsLoading(true);
  ...
  useEffect(() => {
    ...
    // Real-time channel listening to teams, team_members, and team_join_requests
    const channel = supabase
      .channel('unassigned-teams-realtime')
      .on(
        'postgres_changes',
